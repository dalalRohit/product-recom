...
router.post("/links", async function (req, res, next) {
  ...
  if (product in allLinks) {
    ...
    Object.keys(allLinks[product].amazon).map((savedLink) => {
      dataToSend["amazon"].push(allLinks[product].amazon[savedLink]);
    });
    Object.keys(allLinks[product].flipkart).map((savedLink) => {
  ...
  //if product entered for FIRST TIME
    ...
    scrapeLinksFromGoogle(product)
      .then(async (response) => {
        ...
        if (amazonData) {
          amazonLinks = amazonLinks.map((aLink, i) => {
            aLink = {
              ...
              prediction: amazonData[i]["prediction"]
                ? amazonData[i]["prediction"]
                : 0,
        ...
        let { flipkartData } = await flipkartMethod(
          flipkartLinks,
          browser,
          product
        );
        if (flipkartData) {
          flipkartLinks = flipkartLinks.map((fLink, i) => {
            fLink = {
              ...
              prediction: flipkartData[i]["prediction"]
                ? flipkartData[i]["prediction"]
                : 0,
              image: flipkartData[i]["image"]
                ? flipkartData[i]["image"]
                : "image",
            };
...
router.post("/get_features", (req, res, next) => {
  ...
  dataRef.child(`/${prod}/info/amazon`).on(
    "value",
    function (snapshot) {
  ...
  dataRef.child(`/${prod}/info/flipkart`).on(
    "value",
    function (snapshot) {
